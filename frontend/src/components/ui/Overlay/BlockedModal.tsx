...
export const BlockedModal: React.FC<BlockedModalProps> = ({ message, onClose }) => {
    ...
    return (
        <AnimatePresence>
            <aside className="fixed inset-0 z-[1000] flex items-center justify-center p-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
                {/* Backdrop */}
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                ...
                >
                    ...
                    <motion.div
                        ...
                        className="relative"
                    >
                        <figure className="w-24 h-24 bg-red-500 rounded-3xl rotate-12 flex items-center justify-center shadow-2xl shadow-red-500/40 relative z-10 m-0">
                    ...
                    <header className="flex flex-col gap-2 relative z-10">
                        <h2 id="blocked-modal-title" className="text-[2.25rem] font-druk font-black text-text-primary uppercase tracking-tight">
                            SIGNAL COUPE
                        </h2>
