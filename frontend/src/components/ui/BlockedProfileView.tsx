...
export default function BlockedProfileView({ type = 'suspended' }: BlockedProfileViewProps) {
    ...
    return (
        <section className={`flex-1 flex flex-col items-center justify-center p-12 md:p-24 text-center gap-8 ${isSuspended ? 'bg-surface/10' : 'bg-danger/5'} relative overflow-hidden`}>
            {/* Scanner Grid Background */}
            <div className="absolute inset-0 opacity-[0.05] pointer-events-none bg-grid-lines" />

        {/* Visual Icon with Motion */}
        <figure className="relative group/icon">
            <FiSlash className="w-32 h-32 text-danger/5 transition-transform duration-700 group-hover/icon:rotate-90" />
            <motion.div 
                animate={{ opacity: [0.1, 0.3, 0.1], scale: [1, 1.1, 1] }}
                transition={{ duration: 3, repeat: Infinity }}
                className="absolute inset-0 flex items-center justify-center"
            >
                    <FiSlash className="w-20 h-20 text-danger/10 blur-md" />
            </motion.div>
        </figure>
